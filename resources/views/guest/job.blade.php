...
@section('content')
    ...
    <section class="relative w-full overflow-hidden z-0 bg-black">
        <img src="{{ asset('assets/images/lowongan-hero.webp') }}" alt="Intip Keseruan Insan Sinar Sosro Gunung Slamat"
            class="absolute inset-0 w-full h-full object-cover">
        <div class="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-red-900 via-red/80 to-transparent"></div>
        <div class="flex items-center justify-center px-6 py-40 md:px-20 max-w-7xl mx-auto">
            <div class="w-full text-center md:text-left z-10">
                <h1
                    class="text-4xl md:text-5xl lg:text-[2.7rem] text-white font-bold uppercase leading-[1.1] md:max-w-[700px] drop-shadow-lg">
                    Bersama Kami, Menjadi Bagian Sinar Sosro Gunung Slamat
                </h1>
                <p class="text-lg md:text-xl text-white mt-6 md:mt-8 max-w-2xl">
                    Bergabunglah dengan tim kami, baik sebagai talenta muda maupun profesional berpengalaman. Pilih kategori
                    yang sesuai untuk mengembangkan potensi karirmu.
                </p>
                <a onclick="scrollToLowongan(event)"
                    class="max-md:mx-auto hover:cursor-pointer px-6 py-2 rounded-full bg-white text-red-700 font-bold block mt-5 w-max hover:bg-red-700 hover:text-white transition shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                    Lihat Lowongan
                </a>
            </div>
        </div>
    ...
    {{-- SECTION LOWONGAN --}}
    <section class="py-20 bg-white" id="lowongan">
        <div class="max-w-6xl mx-auto px-6">
            ...
            <div class="flex flex-col lg:flex-row gap-4 justify-center items-center mb-10">
                ...
                <div class="relative flex items-center w-full lg:w-2/5 bg-white rounded-full shadow-md overflow-hidden">
                    ...
                    <input id="searchInput" type="text" placeholder="Cari lowongan..."
                    ...
                    <button
                        class="bg-[#B11116] text-white px-8 py-2.5 rounded-full font-bold mr-1.5 hover:bg-red-800 transition shadow-md">
                        Cari
                    </button>
                </div>
                ...
                <div class="relative w-full lg:w-1/5 font-semibold group" x-data="{ open: false }">
                    <select id="filterLocation" {{-- Saat diklik (buka menu), status open jadi true. Saat pilihan dipilih (change), status open jadi false --}} @click="open = !open" @blur="open = false"
                        @change="open = false"
                    ...
                    <div class="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-red-700">
                        <svg {{-- Class ini yang membuat panah berputar seperti di FAQ --}} :class="{ 'rotate-180': open }"
                            class="w-5 h-5 transition-transform duration-300" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                ...
                <div class="relative w-full lg:w-1/5 font-semibold" x-data="{ open: false }">
                    <select id="filterCategory" @click="open = !open" @blur="open = false" @change="open = false"
                        class="appearance-none w-full bg-white border-none rounded-full px-6 py-4 shadow-md text-red-400 focus:outline-none focus:ring-2 focus:ring-red-700/20 cursor-pointer pr-12">
                ...
                {{-- FILTER BIDANG --}}
                <div class="relative w-full lg:w-1/5 font-semibold" x-data="{ open: false }">
                    <select id="filterBidang" @click="open = !open" @blur="open = false" @change="open = false"
                        class="appearance-none w-full bg-white border-none rounded-full px-6 py-4 shadow-md text-red-400 focus:outline-none focus:ring-2 focus:ring-red-700/20 cursor-pointer pr-12">
            ...
            <div id="jobList" class="grid md:grid-cols-2 gap-6">
                ...
                @foreach ($lowongan as $item)
                    <div class="job-card border border-grey-200 rounded-2xl p-6 md:p-8 bg-white shadow-xl inset-shadow-sm inset-shadow-indigo-500 hover:shadow-md transition flex flex-col gap-4"
                        data-title="{{ $item->judul_lowongan }}" data-location="{{ $item->cabang?->nama }}"
                        data-category="{{ $item->kategori }}" data-bidang="{{ $item->bidang }}">
                        ...
                        <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                            ...
                            {{-- RIGHT --}}
                            <div class="flex flex-col gap-3 text-gray-600 mt-2 mb-5 text-sm">
                        ...
                        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <details>
                                    ...
                                    <div class="mt-2 text-gray-600 max-w-none">
                                        {!! $item->jobdesk ?:
                                            '<p class="italic text-gray-400">Deskripsi pekerjaan akan dijelaskan saat proses seleksi.</p>' !!}
                                    </div>
                                </details>
                            </div>
                            ...
                            @if ($sudahMelamar)
                                <button disabled
                                    class="inline-flex items-center justify-center bg-green-600 text-white text-lg font-semibold px-8 py-2 rounded-full cursor-default">
                            ...
                            @else
                                ...
                                <div class="shrink-0">
                                    <form method="POST" action="{{ route('jobs.apply', $item->id) }}"
                                        class="form-lamar">
                                        @csrf
                                        <button type="button" onclick="konfirmasiLamar(this)"
    ...
    <script>
        function scrollToLowongan(event) {
            event.preventDefault();

            const targetSection = document.getElementById('lowongan');
            if (targetSection) {
